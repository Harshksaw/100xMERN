...
exports.resolveInputField = exports.resolveValueFieldType = exports.createInputType = exports.resolveInputArg = void 0;
...
const resolveInputArg = (f) => {
    ...
    return `${(0, description_1.plusDescription)(f.description, '\t')}\t${resolveArgsName(f.name)}${(0, exports.resolveValueFieldType)((0, primitive_1.isTypeScriptPrimitive)(typeName) ? tsp : (0, exports.createInputType)(typeName), f.type.fieldType)}`;
};
exports.resolveInputArg = resolveInputArg;
const createInputType = (t) => `${models_1.INPUTTYPES}["${t}"]`;
...
const resolveInputField = (f, enumsAndScalars) => {
    const { args } = f;
    const typeName = (0, graphql_js_tree_1.getTypeName)(f.type.fieldType);
    const resolvedTypeName = (0, primitive_1.isTypeScriptPrimitive)(typeName) || enumsAndScalars.includes(typeName) ? truthy_1.truthyType : (0, exports.createInputType)(typeName);
    if (args && args.length) {
        return `${f.name}?: [{${args.map(exports.resolveInputArg).join(',')}},${resolvedTypeName}]`;
    }
    return `${(0, description_1.plusDescription)(f.description, '\t')}\t${`${f.name}?` + ':'}${resolvedTypeName}`;
};
exports.resolveInputField = resolveInputField;
